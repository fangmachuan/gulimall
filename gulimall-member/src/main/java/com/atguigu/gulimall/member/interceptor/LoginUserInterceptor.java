...
public class LoginUserInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        //解决：让拦截器所有请求一进来匹配一下，如果现在请求的路径是/member/memberreceiveaddress/info/{id}这个路径，就放行
        String uri = request.getRequestURI();//当前请求
        boolean match = new AntPathMatcher().match("/member/**", uri);//路径的匹配器
        if (match){
            return true; //直接放行
        }
